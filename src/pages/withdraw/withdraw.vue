...
<script>
import BankCardCell from '@/components/bankCard/BankCardCell';
import { HintPanel, MessagePanel, BankCardPanel, WithdrawBox } from '@/components/withdraw';
...
export default {
  ...
  data() {
    return {
      isWithdrawing: false
      // withdrawDeposit: JSON.parse(storage.read('withdrawDeposit')),
    };
  },
  async created() {
    // 将token存入cookie
  ...
  watch: {
    drawDepositInfo(newVal) {
      if (newVal.status == 1) {
        ...
        // this.tips({ text:'提现处理中，请5-10分钟后重试',duration:1000 * 100000000 })
      }
    },
  },
  beforeDestroy(){
