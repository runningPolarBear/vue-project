...
export default {
  ...
  async [SUBMIT_WITHDRAW_DEPOSITINFO]({ commit, dispatch },{ isLoading = false,isToast = true, applyId, productType, bankAccountNo, callBackUrl, money, } = {}) {
    try{
      const response = await dispatch(FETCH_SERVER,{  
  ...
  async [MODIFY_PASSWORD]({ commit, dispatch },{ isLoading = false,isToast = true, callbackUrl } = {}) {
    try{
      const response = await dispatch(FETCH_SERVER,{  
  ...
  async [UN_LOCKING]({ commit, dispatch },{ isLoading = false,isToast = true, callbackUrl, } = {}) {
    try{
      const response = await dispatch(FETCH_SERVER,{  
